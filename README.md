...
- **启用 GSP**：释放 CPU 开销，启用板载的Risc-V处理器硬件（花钱买了为什么不用？），接管由原本CPU负责的初始化，电源管理，资源加载等任务，某些情况会降低输入延迟，提升性能
- **禁用 GSP**：GSP并未在win11平台提供官方支持（Linux平台默认开启），但理应接管GPU内部任务依旧会生效，如果在部分应用，场景出现崩溃或兼容性问题，请禁用它
